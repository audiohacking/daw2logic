...
from .colors import color_sidecar_entry
from .eq import equalizer_to_logic_channel_eq
...
def export_sidecars(logicx_dir: Path, project: Project, report) -> None:
    ...
    manifest: dict = {
        ...
        "notes": [
            ...
            "EQ sidecars map DAWproject Equalizer bands to Logic Channel EQ parameters.",
    ...
    for track in project.tracks:
        ...
        if track.equalizers:
            eq_payloads = [equalizer_to_logic_channel_eq(eq) for eq in track.equalizers]
            entry["equalizers"] = eq_payloads
            eq_dir.mkdir(parents=True, exist_ok=True)
            eq_path = eq_dir / f"{_safe_dir_name(track.name)}.json"
            eq_path.write_text(json.dumps(eq_payloads, indent=2) + "\n")
            entry["eq_sidecar"] = eq_path.relative_to(logicx_dir).as_posix()
            report.warnings.append(
                f"track '{track.name}': EQ exported to sidecar — add Logic Channel EQ and "
                f"match bands from {entry['eq_sidecar']}"
            )
        if track.automation:
        ...
        if track.color:
            entry["track_color"] = color_sidecar_entry(track.color)
        clip_colors = sorted(
            {c.color for c in (*track.midi_clips, *track.audio_clips) if c.color}
        )
        if clip_colors:
            entry["clip_colors"] = [color_sidecar_entry(c) for c in clip_colors]
    ...
    (root / "README.txt").write_text(
        ...
        "EQ: add Logic Channel EQ to each track listed in eq/ and match band settings from the JSON.\n"
